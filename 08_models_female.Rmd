...
# reading dataset
```{r message=FALSE}
dat_model_female <- read_csv("data/data_model.csv") %>% 
...
baseline_model <- 
  ...
  lmer(NA_sum ~ age + gender + beep + day + 
      ...
      (event_unpleasantness_centered | id), data = dat_model_female)
...
lillie.test(dat_model_female$NA_sum)
```
...
trait_model_female <-
  ...
  lmer(NA_sum ~ age + beep + day +
      trait_rumination +
      (1 | id), data = dat_model_female)

tab_model(trait_model_female, show.std = T)
AIC(trait_model_female)
BIC(trait_model_female)
# check_model(trait_model_female)
```
...
```{r}
mean_state_model_female <-

  lmer(NA_sum ~ age + beep + day +
      mean_state_rumi_grand_centered +
      (1 | id), data = dat_model_female)

tab_model(mean_state_model_female, show.std = T)
AIC(mean_state_model_female)
BIC(mean_state_model_female)
# check_model(mean_state_model_female)
```
...
tab_model(state_model_female, show.std = T)
tab_model(trait_model_female, mean_state_model_female, state_model_female, show.std = T, show.aic = T)
AIC(state_model_female)
BIC(state_model_female)
...
full_model_female_negaff <-
  ...
  lmer(NA_sum ~ 
      ...
      (rumination_centered | id), data = dat_model_female,
      control = lmerControl(optCtrl=list(ftol_abs=1e-8,xtol_abs=1e-8))
  )

tab_model(full_model_female_negaff, show.std = T, digits = 3, digits.re = 3)
summary(full_model_female_negaff)
plot_model(full_model_female_negaff, type = "eff")
# check_model(full_model_female_negaff)
check_collinearity(full_model_female_negaff)
plot_model(full_model_female_negaff, rm.terms = c("age", "gender [Male]", "gender [Other]", "beep", "day", "event_unpleasantness_centered"), ci.lvl=0.95)
AIC(full_model_female_negaff)
BIC(full_model_female_negaff)

# checking assumptions
plot_model(full_model_female_negaff)
```

# full model posaff
```{r}
full_model_female_posaff <-
...
tab_model(full_model_female_posaff, show.std = T, digits = 3, digits.re = 3)
summary(full_model_female_posaff)
plot_model(full_model_female_posaff, type = "eff")
# check_model(full_model_female_posaff)
check_collinearity(full_model_female_posaff)
plot_model(full_model_female_posaff, rm.terms = c("age", "gender [Male]", "gender [Other]", "beep", "day", "event_unpleasantness_centered"), ci.lvl=0.95)
AIC(full_model_female_posaff)
BIC(full_model_female_posaff)

# checking assumptions
plot_model(full_model_female_posaff)
```
...
#       (event_unpleasantness_centered * rumination_centered | id), data = dat_model_female
#       # control = lmerControl(optCtrl=list(ftol_abs=1e-8,xtol_abs=1e-8))
#   )
# 
...
## running bootstrap
```{r}
# case_boot_full_model_female_negaff <-
#   bootstrap(
#     full_model_female_negaff,
#     .f = fixef,
#     type = "case",
...
#   )
# 
# saveRDS(case_boot_full_model_female_negaff, "bootstrapping/case_boot_full_model_female_negaff.rds")
```

## reading bootstrap file and extracting results into a list
```{r}
case_boot_full_model_female_negaff <- readRDS("bootstrapping/case_boot_full_model_female_negaff.rds")

list_case_boot_full_model_female_negaff <-
  list(case_boot_full_model_female_negaff)
```

## extracting bootstrapped coefficients
```{r paged.print=TRUE}
case_boot_ci_full_model_female_negaff_basic  <-
  list_case_boot_full_model_female_negaff %>%
  map(confint, type = "basic")
case_boot_ci_full_model_female_negaff_basic

case_boot_ci_full_model_female_negaff_perc  <-
  list_case_boot_full_model_female_negaff %>%
...
## running bootstrap
```{r}
# case_boot_full_model_female_posaff <-
#   bootstrap(
#     full_model_female_posaff,
#     .f = fixef,
#     type = "case",
...
#   )
# 
# saveRDS(case_boot_full_model_female_posaff, "bootstrapping/case_boot_full_model_female_posaff.rds")
```

## reading bootstrap file and extracting results into a list
```{r}
case_boot_full_model_female_posaff <- readRDS("bootstrapping/case_boot_full_model_female_posaff.rds")

list_case_boot_full_model_female_posaff <-
  list(case_boot_full_model_female_posaff)
```

## extracting bootstrapped coefficients
```{r paged.print=TRUE}
case_boot_ci_full_model_female_posaff_basic  <-
  list_case_boot_full_model_female_posaff %>%
  map(confint, type = "basic")
case_boot_ci_full_model_female_posaff_basic

case_boot_ci_full_model_female_posaff_perc  <-
  list_case_boot_full_model_female_posaff %>%
  map(confint, type = "perc")
case_boot_ci_full_model_female_posaff_perc
```
