...
ggplot(dat_unfiltered_figure) +
  geom_histogram(aes(esm_compl_final), bins = 40)
```
...
corr_rumi <- dat_processed %>% 
  mutate(mean_state_rumi_scaled = scale(mean_state_rumi, center = F),
         rum_cerq_scaled = scale(rum_cerq, center = F)
         ) %>% 
  select(id, mean_state_rumi_scaled, rum_cerq_scaled) %>% 
  distinct() %>% 
  select(-id)

shapiro.test(corr_rumi$rum_cerq)
shapiro.test(corr_rumi$mean_state_rumi)
# normality not assumed

cor.test(corr_rumi$rum_cerq, corr_rumi$mean_state_rumi, method = "spearman")

ggplot(corr_rumi, aes(rum_cerq_scaled, mean_state_rumi_scaled)) +
