...
```{r}
full_model_negaff <- readRDS("saved_models/full_model_negaff.rds")
full_model_negaff_2pred <- readRDS("saved_models/full_model_negaff_2pred.rds")
```
...
```{r}
# cases_boot_full_model_negaff <-
#   bootstrap(
#     full_model_negaff_2pred,
#     .f = fixef,
#     type = "case",
#     B = 5000 ,
...
#   )
# 
# saveRDS(cases_boot_full_model_negaff, "bootstrapping/cases_boot_full_model_negaff_2pred.rds")
```

### reading bootstrap file and extracting results into a list

```{r}
cases_boot_full_model_negaff_2pred <- readRDS("bootstrapping/cases_boot_full_model_negaff_2pred.rds")

list_cases_boot_full_model_negaff_2pred <-
  list(cases_boot_full_model_negaff_2pred)
```

### extracting bootstrapped coefficients

```{r paged.print=TRUE}
# cases_boot_ci_full_model_negaff  <-
#   list_cases_boot_full_model_negaff %>%
#   map(confint, type = "perc")
# cases_boot_ci_full_model_negaff

boot_negaff_replicates_2pred <- cases_boot_full_model_negaff_2pred$replicates
boot_negaff_confints_2pred <- boot_negaff_replicates_2pred %>% 
...
boot_negaff_estimates_2pred <- as_tibble(cases_boot_full_model_negaff_2pred$observed) %>%
  mutate(term = c("intercept", "meanStateRumiGrandCentered", "traitRumination")) %>% 
  select(term, estimate = value) %>% 
  full_join(boot_negaff_confints_2pred, by = "term") %>%
  mutate(
...
```{r}
residual_boot_full_model_negaff <-
  bootstrap(
    full_model_negaff,
    .f = fixef,
    type = "residual",
...
saveRDS(residual_boot_full_model_negaff, "bootstrapping/residual_boot_full_model_negaff.rds")
```

### reading bootstrap file and extracting results into a list

```{r}
residual_boot_full_model_negaff <- readRDS("bootstrapping/residual_boot_full_model_negaff.rds")

list_residual_boot_full_model_negaff <-
  list(residual_boot_full_model_negaff)
```

### extracting bootstrapped coefficients

```{r paged.print=TRUE}
residual_boot_ci_full_model_negaff_basic  <-
  list_residual_boot_full_model_negaff %>%
  map(confint, type = "basic")
residual_boot_ci_full_model_negaff_basic

residual_boot_ci_full_model_negaff_perc  <-
  list_residual_boot_full_model_negaff %>%
  map(confint, type = "perc")
residual_boot_ci_full_model_negaff_perc
```
...
dat_cases_boot_full_model_negaff_2pred_maineff <- 
  list_cases_boot_full_model_negaff_2pred %>% 
  map("replicates") %>% 
  map(
    ~tibble(
...
dat_cases_boot_full_model_negaff_maineff_2pred_long <- 
  dat_cases_boot_full_model_negaff_2pred_maineff %>% 
  pivot_longer(c(trait_rumination, mean_state_rumination,), names_to = "rumination_measure", values_to = "rumination_intensity")

# writing .csv
write.csv(dat_cases_boot_full_model_negaff_maineff_2pred_long, "data/dat_cases_boot_full_model_negaff_2pred_maineff_long.csv")


# plotting
## density plot
coef_cases_boot_full_model_negaff_maineff <- 
  dat_cases_boot_full_model_negaff_maineff_long %>% 
...
## violin plot
violin_cases_boot_full_model_negaff_maineff <- 
  dat_cases_boot_full_model_negaff_maineff_long %>% 
...
```{r echo=TRUE}
# extracting results into a tibble
dat_residual_boot_full_model_negaff_maineff <- 
  list_residual_boot_full_model_negaff %>% 
  map("replicates") %>% 
  map(
    ~tibble(
...
# rearranging tibble format
dat_residual_boot_full_model_negaff_maineff_long <- 
  dat_residual_boot_full_model_negaff_maineff %>% 
  pivot_longer(c(trait_rumination, mean_state_rumination, state_rumination), names_to = "rumination_measure", values_to = "rumination_intensity")


# plotting
coef_residual_boot_full_model_negaff_maineff <- 
  dat_residual_boot_full_model_negaff_maineff_long %>% 
  ggplot(aes(rumination_intensity, fill = rumination_measure, linetype = rumination_measure)) +
     geom_vline(xintercept = 0, color = "black", linetype = 2, alpha = .5, size = 1) + 
     geom_density(alpha = .7, outline.type = "both") +
...
coef_residual_boot_full_model_interac <- 
  ...
  ggplot(aes(rumination_intensity, fill = rumination_measure, linetype = rumination_measure)) +
     ...
     geom_density(alpha = .7, outline.type = "both") +
     labs(
       x = "Boostrapped raw estimates for the moderation of\nemotional reactivity by different measures of rumination",
       y = "Density",
       title = "",
       fill = "",
       linetype = ""
...
coef_parametric_boot_full_model_interac <- 
  ...
  ggplot(aes(rumination_intensity, fill = rumination_measure)) +
     ...
     geom_density(alpha = .65, color = NA) +
     labs(
       x    = "Boostrapped raw estimates of the moderation of\nemotional reactivity by different measures of rumination",
       y = "Density",
       fill = "",
       title    = ""
       ) +
...
### interactions (moderation of emotional reactivity) [HU]
...
# plotting
coef_parametric_boot_full_model_interac_HU <- 
  dat_parametric_boot_full_model_interac_long %>% 
  ggplot(aes(rumination_intensity, fill = rumination_measure)) +
     geom_vline(xintercept = 0, color = "black", linetype = 2, alpha = .5, size = 1) + 
     geom_density(alpha = .7, color = NA) +
     labs(
       x     = "Rumináció-mutatók érzelmi reaktivitást\nmoderáló erejének bootstrappelt becslése",
       y     = "Sűrűség",
       fill  = "",
       title = ""
       ) +
