...
# computing aggregated mean of state rumintaion
```{r}
dat_cleaned <- dat_cleaned %>% 
  group_by(id) %>% 
  mutate(mean_state_rumi = mean(regu_rumination, na.rm = TRUE),
         rumination_centered = scale(regu_rumination, center = T, scale = F)
         ) %>% 
  ungroup()

dat_cleaned <- dat_cleaned %>% 
  mutate(mean_state_rumi_grand_centered = scale(mean_state_rumi, center = T, scale = F))
```
...
dat_model <- dat_cleaned %>%
  select(id:edu, event_unpleasantness_centered, trait_rumination = rumination, NA_within:rumination_centered, mean_state_rumi_grand_centered)
```
