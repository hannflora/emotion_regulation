...
scores_cfa_acceptance <-
  dat_cfa_accept %>%
  distinct() %>%
  cbind(lavPredict(fit_accept, newdata = dat_cfa_accept[1:4])) %>%
...
dat_model <- dat_cleaned %>%
  select(id:edu, event_unpleasantness_centered, trait_rumination, trait_reappraisal, trait_acceptance, NA_within:NA_sum, NA_sum_log, PA_within:PA_sum, mean_state_rumi, rumination_centered, mean_state_rumi_grand_centered, mean_state_reapp, reapp_centered, mean_state_reapp_grand_centered, mean_state_reflect, reflect_centered, mean_state_reflect_grand_centered) %>% 
